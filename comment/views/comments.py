...
class CreateComment(CanCreateMixin, BaseCommentView):
    ...
    def form_valid(self, form):
        ...
        if self.comment.parent:
            ...
            if parent_email in [author_email, parent_email]:
                parent_email = False
        if author_email:
            email = EmailMessage(
                "دیدگاه جدید",
        ...
        if user_email:
            email = EmailMessage(
                "دیدگاهتان دریافت شد.",
                "دیدگاهتان دریافت شد و به زودی به آن پاسخ داده خواهد شد.",
                to=[user_email]
            )
            email.send()
        if parent_email:
            email = EmailMessage(
                "پاسخ به دیدگاه شما ارسال شد.",
                "پاسخ به دیدگاه شما در مقاله {} ارسال شد به منظور مشاهده پاسخ بر روی لینک زیر کلیک کنید:\n{}{}".format(
