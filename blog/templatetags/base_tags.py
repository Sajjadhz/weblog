# simple tag read more from https://docs.djangoproject.com/en/3.1/howto/custom-template-tags/
...
def hot_articles():
    ...
    return {
        "articles": Article.objects.published().annotate(
            count=Count('comments', filter=Q(comments__posted__gte=last_month) and Q(comments__content_type_id=content_type_id))).order_by('-count', '-publish')[:5],
